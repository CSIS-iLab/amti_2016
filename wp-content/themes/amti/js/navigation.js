...
jQuery(document).ready(function($) {
	...
	if ($(' .anchor-links').length) {
		$('.single-island-tracker .entry-content h2, .single-island-tracker .entry-content h3').each(function(i) {

			$headingId = ($(this).text() + '-heading').replace(/\s/g,'');

			$(this).attr('id', $headingId);

			$('.anchor-links').append('<a href="#' + $headingId + '"><button>' + $(this).text() + '</button></a>');
		});
	};
